    ...
    while True:
        ...
        for i in obj_list:
            if i.starting_period != i.ending_period:
                # print(i.starting_period.astimezone(tz=local_time))
...
def test_send_sms_notification(self):
    ...
    headers["Authorization"] = f"Bearer {access_token}"
    headers["Content-Type"] = "application/json"
    d = datetime.datetime(2022,8,3,0,0,0,tzinfo=pytz.UTC)
    schedule_group = ScheduleGroup.objects.filter(starting_period__gte=d).select_related().order_by('group_name')
    ...
    for schedule_data in schedule_group:
        if schedule_data.group_name.name != 'A' and schedule_data.group_name.name != 'M':
            continue
